...
export const ProjectCard = ({
  ...
  return (
    <article className="flex flex-col justify-between bg-white shadow-projectCard rounded-3xl overflow-hidden h-full">
      <div>
        <Image
          width={800}
          height={800}
          src={imageUrl || "/profile-logo.png"}
          alt="Specific Alt"
          className="w-full h-60 object-cover bg-center"
        />
      </div>
      <div className="flex flex-col gap-3 p-4">
        <h3 className="text-block font-bold text-lg">{title}</h3>
        <p className="text-content-gray">{description}</p>
        {!!skills && skills.length > 0 && (
          <span className="text-base">
            مهارت فنی:
            <span className="text-sm md:text-base lg:text-md text-content-gray">
              {skills.map((skill, index) => {
                const currentIndex = index + 1;
                return `${skill}${skills.length == currentIndex ? "" : ", "}`;
              })}
            </span>
          </span>
        )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-3">
        {!!onlineUrl && (
          <Link
            href={onlineUrl}
            target="_blank"
            className="group flex items-center justify-between gap-3 py-2 px-5 rounded-lg hover:bg-black hover:text-white transition-[background] duration-0"
          >
            <svg
              width="20"
              height="20"
            ...
            <p className="text-nowrap">مشاهده آنلاین</p>
          </Link>
        )}
        {!!sourceUrl && (
          <Link
          ...
          >
            ...
            <p className="text-nowrap">سورس پروژه</p>
          </Link>
        )}
      </div>
    </article>
